...
\begin{lstlisting}[language=Verilog]
    \code{input wire [4:0] operation},  
    \code{input wire vsync}, 
    \code{input reg [10:0] framenumber}, 
    \code{output reg [2:0] currentstate[0:9][0:19]}. 
    \code{output reg [7:0] score},
    \code{output reg gameover}
\end{lstlisting}}
...
along with the gamescore and a gameover state. The vsync acts as the clock that drives the game.

Further, there are several interal components 
\begin{lstlisting}[language=Verilog]
    \code{ reg [2:0] tetrimino};
    \code{reg frozen};
    \code{reg [2:0] num_deleted};
    \code{reg [4:0] centerofmass [0:1][0:3]}
\end{lstlisting}}
